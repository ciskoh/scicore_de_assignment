...
from src import datamodels as dm
...
def execute_exercise_1(src_folder, db_path):
    ...
    t3_df.columns = [
        c.lower().replace(" ", "_").replace("#", "n") for c in t3_df.columns
    ]
    ...
    t4_df.columns = [
        c.lower().replace(" ", "_").replace("#", "n") for c in t4_df.columns
    ]
...
def execute_exercise_3(
    db_path: str, taxonomy: str = "573", annotation: str = "Imipenem"
):
    """
    ...
    print(
        "ingested table uni_prot",
        f"records_n: {len(my_table)}",
        f" attributes: {my_table.columns}",
    )
...
def main_wrapper():
    ...
    args.add_argument(
        ...
        type=int,
        required=True,
        help="exercise number to execute, Int from 1 to 3",
    )
    args.add_argument(
        "-s",
        "--source",
        ...
        help="source folder of raw data for exercise 1",
    )
    args.add_argument(
        "-d",
        "--pathtodb",
        ...
        help="path to database location",
    )
    args.add_argument(
        "-p",
        "--species",
        ...
        required=False,
        default="Kleb",
        help="Species to query for exercise 2'",
    )
    args.add_argument(
        "-t",
        "--taxonomyid",
        ...
        required=False,
        default="573",
        help="taxonomy code for exercise 3",
    )
    args.add_argument(
        "-a",
        "--annotation",
        ...
        required=False,
        default="Imipenem",
        help="annotation keyword to query for exercise 4",
    )
    ...
    if args.exercise == 1:
        ...
        raise ValueError(
            "choose an exercise between 1 and 3. You chose : {args.exercise}"
        )
